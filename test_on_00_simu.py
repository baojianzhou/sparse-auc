...
def test_solam_sector_full():
    def get_full_matrix(x_tr_vals, x_tr_inds, x_tr_posis, x_tr_lens):
        data_mat = np.zeros(shape=(len(x_tr_lens), data['p']), dtype=float)
        for i in range(len(x_tr_lens)):
            sub_indices = x_tr_inds[x_tr_posis[i]:x_tr_posis[i] + x_tr_lens[i]]
            sub_vals = x_tr_vals[x_tr_posis[i]:x_tr_posis[i] + x_tr_lens[i]]
            data_mat[i][sub_indices] = sub_vals
        return data_mat
    ...
    list_xi = np.arange(1, 101, 9, dtype=float)
    list_r = 10 ** np.arange(-1, 6, 1, dtype=float)
    best_auc, run_id, fold_id, num_passes = 0.0, 0, 0, 5
    data_mat = get_full_matrix(data['x_tr_vals'], data['x_tr_indices'],
                               data['x_tr_posis'], data['x_tr_lens'])
    s_time = time.time()
    for para_xi, para_r in product(list_xi, list_r):
        tr_index = data['run_%d_fold_%d' % (run_id, fold_id)]['tr_index']
        te_index = data['run_%d_fold_%d' % (run_id, fold_id)]['te_index']
        re = c_algo_solam(np.asarray(data_mat[tr_index], dtype=float),
                          np.asarray(data['y_tr'][tr_index], dtype=float),
                          para_xi, para_r, num_passes, 0)
        wt = np.asarray(re[0])
        wt_bar = np.asarray(re[1])
        auc_wt = roc_auc_score(y_true=data['y_tr'][te_index],
                               y_score=np.dot(data_mat[te_index], wt))
        auc_wt_bar = roc_auc_score(y_true=data['y_tr'][te_index],
                                   y_score=np.dot(data_mat[te_index], wt_bar))
        print(para_xi, para_r, auc_wt, auc_wt_bar, time.time() - s_time)


def test_solam_sector_sparse():
    ...
    for para_xi, para_r in product(list_xi, list_r):
        ...
        re = c_algo_solam_sparse(np.asarray(sub_x_values, dtype=float),
