...
void _algo_fsauc_sparse(const double *x_tr_vals,
    ...
    double kappa = 1.0, t = 1., global_n = data_n * para_num_passes;
    ...
    for (int k = 0; k < m; k++) {
        ...
        for (int kk = 0; kk < n0; kk++) {
            ...
            alpha += gamma * galpha;
            for (int j = 0; j < K; j++) { // project: w,a,b
            ...
            double tmp = fmin(fmin(D0 + alpha1, 2. * R * kappa), alpha); //project: alpha
