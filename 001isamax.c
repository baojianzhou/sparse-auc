...
for(j=0;j<n;j++) x[j]=( float )j; // x={0 ,1 ,2 ,3 ,4 ,5}
printf("x: ");
for(j=0;j<n;j++) printf (" %4.0f,",x[j]); // print x
printf ("\n");
// on the device
...
// find the smallest index of the element of d_x with maximum absolute value
stat=cublasIsamax(handle,n,d_x,1,&result);
printf ("max |x[i ]|:%4.0 f\n",fabs (x[result -1])); // print max {|x [0]| ,... ,| x[n -1]|}
// find the smallest index of the element of d_x with minimum absolute value
